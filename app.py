...
def getTotalMonthCost():
        ...
        res_cost = 0
        for key, value in result_exp[startMonth].items():
            res_cost += value
...
def investmentPlanner():
    ...
    Y = (X/(tenure * 12))
    inflation_amt = (5/100) * X * tenure * 12
    rate = 0
    if Y <= 1000:
        rate = 8
    ...
    SIP = round(X/(((pow(1+montly_rate_of_return, tenure*12) - 1)/montly_rate_of_return) *(1+montly_rate_of_return)))
    # total_money_invested = rate * tenure*12
    # safety_net = upfront_amt * rate
    return "<h1> {0} | {1} | {2} | {3} | {4} | {5}</h1>".format(X, Y, rate, montly_rate_of_return, SIP, inflation_amt)
...
def personalBudget():
        ...
        for i in range(int(priority)):
            res_obj[num_char[i]] = total * int(priority_division[str(priority)][i])/100

        print(res_obj)
        return "<h1> Personal Budget </h1>"
